...
uint16_t anchors[4] = {0x6023, 0x6041, 0x605A, 0x605C};     // the network id of the anchors: change these to the network ids of your anchors.
int32_t heights[4] = {1400, 1400, 1400, 1400};              // anchor z-coordinates in mm
...
void setup(){
  ...
  int status_ret = Pozyx.clearDevices();
  //printf(" get clearDevices feedback %d \n", status_ret);
...
void printCalibrationResult(){
  ...
  if(list_size == 0){
	...
	Serial.println((const char *)((Pozyx.getSystemError()).data())); 
    return;
  }
...
{
	Serial.println("start to exec localize program!");
	setup();
	while (1)
	{
		loop();
